...
int main (int argc, char** argv) {
	printf("\n\n***************************************\n");
	printf("cardtest3: test playMine function\n");
	printf("***************************************\n");
  ...
  // toFlag = 2 : add to hand
	...
	gainCard(mine, &post, 2, 0);
	gainCard(copper, &post, 2, 0);
	...
	asserttrue(playMine(&post, handpos + 1, gold, handpos) == 0, "playMine returns 0");
	...
	if(!asserttrue(post.hand[0][handpos] != mine, "Player 1: Mine card is no longer in player's hand"))
	{
		//since the test was failed, the added treasure cards are offset by 1
		handpos++;		
	}	
	asserttrue(post.hand[0][handpos] == gold, "Player 1: Copper card is now a Gold card");	
	asserttrue(post.handCount[0] == pre.handCount[0] + 1, "Player 1: Only one card added");
